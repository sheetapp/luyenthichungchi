    ...
    useEffect(() => {
        ...
        const loadQuestions = async () => {
            ...
            try {
                let query = supabase
                    .from('questions')
                    .select('id_cauhoi, stt, cau_hoi, chuyen_nganh')
                    .eq('hang', selectedHang)
                    .eq('phan_thi', selectedPhanThi)
                    .order('stt', { ascending: true })

                // Filter by chuyen_nganh only if Phần thi is Chuyên môn
                ...
                if (!error && data) {
                    // Map id_cauhoi to id for consistency
                    const mappedData = data.map(q => ({
                        id: q.id_cauhoi,
                        stt: q.stt,
                        cau_hoi: q.cau_hoi,
                        chuyen_nganh: q.chuyen_nganh
                    }))
        ...
        loadQuestions()
        setSelectedQuestionId('') // Reset selection when filters change
