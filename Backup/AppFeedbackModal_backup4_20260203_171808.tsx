...
import { Star, Send, X, MessageSquare, Loader2, ShieldCheck, AlertTriangle, Search } from 'lucide-react'
...
export function AppFeedbackModal({ isOpen, onClose, user }: AppFeedbackModalProps) {
    ...
    const [searchMode, setSearchMode] = useState<'name' | 'stt'>('name')

    const selectedType = FEEDBACK_TYPES.find(t => t.value === feedbackType)

    // Search questions by name
    useEffect(() => {
        if (!selectedType?.requiresQuestion || searchMode !== 'name' || !questionSearch.trim()) {
            setSearchResults([])
        ...
        const searchQuestions = async () => {
            const { data, error } = await supabase
                ...
                .ilike('cau_hoi', `%${questionSearch}%`)
                .limit(10)

            if (!error && data) {
                // Filter by chuyen_nganh if Phần thi is Chuyên môn
                const filtered = selectedPhanThi === 'Câu hỏi Chuyên môn'
                    ? data.filter(q => q.chuyen_nganh === selectedChuyenNganh)
                    : data
    ...
    }, [questionSearch, selectedHang, selectedPhanThi, selectedChuyenNganh, selectedType, searchMode])

    // Fetch question by STT
    useEffect(() => {
        if (!selectedType?.requiresQuestion || searchMode !== 'stt' || !questionSTT) {
            if (searchMode === 'stt') {
                setQuestionId(null)
        ...
        const fetchQuestion = async () => {
            ...
            let query = supabase
                .from('questions')
                .select('id, chuyen_nganh, stt')
                .eq('hang', selectedHang)
                .eq('phan_thi', selectedPhanThi)
                .eq('stt', stt)

            // Filter by chuyen_nganh if Phần thi is Chuyên môn
            if (selectedPhanThi === 'Câu hỏi Chuyên môn') {
        ...
        return () => clearTimeout(debounce)
    }, [questionSTT, selectedHang, selectedPhanThi, selectedChuyenNganh, selectedType, searchMode])

    const handleQuestionSelect = (question: Question) => {
        setQuestionId(question.id)
        setQuestionChuyenNganh(question.chuyen_nganh)
        setQuestionSTT(question.stt.toString())
        setQuestionSearch(question.cau_hoi)
        setShowSearchResults(false)
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (selectedType?.requiresQuestion) {
            if (searchMode === 'name' && !questionSearch.trim()) {
                alert('Vui lòng tìm và chọn câu hỏi!')
                return
            }
            if (searchMode === 'stt' && !questionSTT) {
                alert('Vui lòng nhập số thứ tự câu hỏi!')
                return
            }
            if (!questionId) {
                alert('Không tìm thấy câu hỏi với thông tin đã nhập. Vui lòng kiểm tra lại.')
                return
            }
        }
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .insert({
                    ...
                    chuyen_nganh: selectedType?.requiresQuestion ? (selectedPhanThi === 'Câu hỏi Chuyên môn' ? selectedChuyenNganh : questionChuyenNganh) : null
                })
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    {selectedType?.requiresQuestion && (
                        <div className="space-y-4 p-4 bg-orange-500/5 border border-orange-500/20 rounded-xl">
                            ...
                            {/* Chuyên ngành (only for Chuyên môn) */}
                            {selectedPhanThi === 'Câu hỏi Chuyên môn' && (
                                <div className="space-y-2">
                            ...
                            {/* Phần thi */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-apple-text">
                                    {selectedPhanThi === 'Câu hỏi Chuyên môn' ? '3' : '2'}. Chọn Phần thi
                                </label>
                                <select
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-apple-text">
                                    {selectedPhanThi === 'Câu hỏi Chuyên môn' ? '4' : '3'}. Tìm câu hỏi
                                </label>
                                <div className="flex gap-2 mb-2">
                                    <button
                                        ...
                                        className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${searchMode === 'name'
                                                ? 'bg-apple-blue text-white'
                                                : 'bg-apple-bg border border-apple-border text-apple-text'
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${searchMode === 'stt'
                                                ? 'bg-apple-blue text-white'
                                                : 'bg-apple-bg border border-apple-border text-apple-text'
                                            }`}
                                    >
                                    ...
                                    <div className="relative">
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-10 pr-3 py-2 bg-apple-bg border border-apple-border rounded-lg text-sm text-apple-text outline-none focus:ring-2 focus:ring-apple-blue/20"
                                            />
                                        </div>
                                        {/* Search Results Dropdown */}
                                        {showSearchResults && searchResults.length > 0 && (
                                            <div className="absolute z-50 w-full mt-1 bg-apple-card border border-apple-border rounded-lg shadow-xl max-h-60 overflow-y-auto">
                                                {searchResults.map((q) => (
                                                    <button
                                                        key={q.id}
                                                        type="button"
                                                        onClick={() => handleQuestionSelect(q)}
                                                        className="w-full px-3 py-2 text-left hover:bg-apple-bg transition-colors border-b border-apple-border last:border-0"
                                                    >
                                                        <div className="flex items-start gap-2">
                                                            <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-xs font-semibold shrink-0">
                                                                STT: {q.stt}
                                                            </span>
                                                            <span className="text-xs text-apple-text line-clamp-2">
                                                                {q.cau_hoi}
                                                            </span>
                                                        </div>
                                                    </button>
                                        ...
                                        {questionId && searchMode === 'name' && (
                                            <p className="text-xs text-green-600 mt-1">✓ Đã chọn câu hỏi STT: {questionSTT}</p>
                                        )}
                                    </div>
                                )}
                                ...
                                {searchMode === 'stt' && (
                                    <div>
                                        ...
                                        {questionSTT && questionId && (
                                            <p className="text-xs text-green-600 mt-1">✓ Đã tìm thấy câu hỏi</p>
                                        )}
                                        {questionSTT && !questionId && (
                                            <p className="text-xs text-red-600 mt-1">✗ Không tìm thấy câu hỏi</p>
                                        )}
                                    </div>
                                )}
                            </div>
