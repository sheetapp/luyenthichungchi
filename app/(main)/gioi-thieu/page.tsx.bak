...
export default function AboutPage() {
    return (
        <div className="min-h-screen bg-white font-sans text-[#1d1d1f] selection:bg-[#007AFF]/20">
            ...
            <section className="py-24 bg-[#F5F5F7] border-t border-black/5 overflow-hidden">
                <style jsx>{`
                    @keyframes scroll {
                ...
                <div className="space-y-12">
                    <div className="max-w-6xl mx-auto px-6 text-center space-y-4">
                        ...
                        <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Được tin dùng bởi các kỹ sư.</h2>
                    </div>

                    <div className="w-full">
                        <div className="flex gap-8 w-max animate-scroll pl-8">
...
function ReviewCard({ name, content, avatar, stars, badgeLabel, badgeColor }: any) {
    return (
        <div className="p-8 rounded-[24px] bg-white border border-black/5 shadow-sm space-y-6 flex flex-col justify-between w-[350px] shrink-0 hover:scale-105 transition-transform duration-300">
            <div className="space-y-4">
                ...
                <p className="text-[#1d1d1f] font-medium leading-relaxed italic line-clamp-3">"{content}"</p>
            </div>

            <div className="flex items-center gap-4 pt-4 border-t border-black/5">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-md bg-gradient-to-br from-gray-700 to-black`}>
                    {avatar}
                </div>
                <div>
                    <div className="font-bold text-sm text-[#1d1d1f]">{name}</div>
                    <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider mt-1 ${badgeColor}`}>
