...
import { ProfileVerificationModal } from '@/components/exam/ProfileVerificationModal'
...
export default function ThiThuPage() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            const { data, error } = await supabase
                .from('profiles')
                .select('display_name, phone, job_title, gender')
                .eq('id', user.id)
                .single()
    ...
    // Handle start exam with profile check
    const handleStartExam = (category: string) => {
        if (!isProfileComplete(profile)) {
            ...
            router.push(`/thi-thu/${encodeURIComponent(category)}`)
        }
    }
